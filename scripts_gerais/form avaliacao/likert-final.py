...
def quebrar_label(texto, largura=35):
    """Quebra o texto em múltiplas linhas respeitando largura máxima."""
    import textwrap
...
def grafico_divergente(labels, matriz, titulo, ax):
    ...
    for i_row in range(n):
        ...
        ax.barh(i_row, matriz[i_row, 2], left=-matriz[i_row, 2]/2,
                color=CORES["Neutro"], edgecolor="white", linewidth=0.3, height=0.6)
    ...
    for i_row in range(n):
        ...
        for j in [3, 4]:
            ...
            if w >= 5:
                ax.text(cx, i_row, f"{w:.0f}%", ha="center", va="center",
                        fontsize=12,
                        color="white" if j == 4 else "#333333",
                        fontweight="bold")
            x += w
        w = matriz[i_row, 2]
        if w >= 5:
    ...
    labels_quebradas = [quebrar_label(l, largura=35) for l in labels]
    ax.set_yticklabels(labels_quebradas, fontsize=14, fontweight="semibold",
                       linespacing=1.4)

    ax.set_xlabel("Porcentagem (%)", fontsize=14, fontweight="medium")
    ax.set_title(titulo, fontsize=24, fontweight="bold", pad=14)
    ...
    # Espaçamento vertical entre labels
...
# ── Alunos ────────────────────────────────────────────────────────────────
fig1, ax1 = plt.subplots(figsize=(16, len(LABELS_ALUNO) * 1.4 + 3))
...
ax1.legend(handles=patches, fontsize=13, loc="upper left",
           ncol=1, framealpha=0.8, bbox_to_anchor=(1.01, 1), borderaxespad=0)
plt.tight_layout(pad=1.5)
plt.subplots_adjust(left=0.38, right=0.72, top=0.90, bottom=0.06)
...
# ── Coaches ───────────────────────────────────────────────────────────────
fig2, ax2 = plt.subplots(figsize=(16, len(LABELS_COACH) * 1.4 + 3))
...
ax2.legend(handles=patches, fontsize=13, loc="upper left",
           ncol=1, framealpha=0.8, bbox_to_anchor=(1.01, 1), borderaxespad=0)
plt.tight_layout(pad=1.5)
plt.subplots_adjust(left=0.38, right=0.72, top=0.90, bottom=0.06)
...
def heatmap_personas(rows_grupo, get_vals, labels, titulo, path):
    ...
    ax_h.set_xticklabels(personas, rotation=30, ha="right", fontsize=12)
    ax_h.set_yticks(range(n_q))
    ax_h.set_yticklabels(labels, fontsize=8)
    ax_h.set_xlabel("Perfil", fontsize=12)
    ax_h.set_ylabel("Perguntas", fontsize=12)
    ax_h.set_title(titulo, fontsize=13, fontweight="bold", pad=12)
